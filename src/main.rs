...
mod tests {
    ...
    fn test_endpoint_addr() {
        let ep = Endpoint { enabled: true, host: "127.0.0.1".to_string(), port: 9090 };
        assert_eq!(ep.addr(), "127.0.0.1:9090");
    }
    ...
    #[test]
    fn test_build_output_file_base() {
        let base = build_output_file_base("my source", "sql_test");
        assert!(base.starts_with("my_source_sql_test_"));
    }
}
